...
-- A type for well-quasi-orderings, together with a small interface. I don't
-- fully understand why, but this program doesn't work when Omega is a record
-- type.
--
...
record LTN (wqo1 : Omega) (wqo2 : Omega) where
    constructor LTN_prf
    m : Set wqo1 -> Set wqo2
    ub : Set wqo2
    ord_pres : (x, y : Set wqo1) -> ltn wqo1 x y -> ltn wqo2 (m x) (m y)
    bnd : (x : Set wqo1) -> ltn wqo2 (m x) ub

tran_LTN : tran LTN
tran_LTN a b c a_ltn_b b_ltn_c = LTN_prf m_ac ub_ac ord_pres_ac bnd_ac where
    m_ac : Set a -> Set c
...
namespace wf_LTN_prf
    parameters (p : Omega -> Type, nempty_prf : nempty p, no_lb_prf : no_lb LTN p)
        ...
        a : Omega
        a = fst (no_lb_prf b b_in_p)

        a_in_p : p a
        a_in_p = fst (snd (no_lb_prf b b_in_p))

        a_ltn_b : LTN a b
        a_ltn_b = snd (snd (no_lb_prf b b_in_p))
